...
export const DashboardPage: React.FC<DashboardPageProps> = ({ user, onNavigate }) => {
  ...
  const [sendingRolls, setSendingRolls] = useState<Record<string, boolean>>({});
  const [toastMessage, setToastMessage] = useState<string | null>(null);
  const [toastType, setToastType] = useState<'success' | 'simulated' | 'error'>('success');
  ...
  const handleNotifyStudent = async (rollNo: string, recipientPhone?: string) => {
    setSendingRolls((prev) => ({ ...prev, [rollNo]: true }));
    try {
      const result = await ApiClient.post<{
        student_roll: string;
        status: string;
        delivery_results: Record<string, any> | null;
      ...
      if (s === 'delivered') {
        ...
        setToastMessage(`✓ Reminder sent via Twilio for ${rollNo} — ${result.status}`);
      } else if (s === 'simulated') {
        setToastType('simulated');
        setToastMessage(`⚡ Reminder logged (simulated) for ${rollNo} — Twilio not configured`);
      } else {
        setToastType('error');
        setToastMessage(`⚠ Reminder dispatch issue for ${rollNo} — ${result.status}`);
      }
    } catch (err: any) {
      setToastType('error');
      setToastMessage(`Failed to send reminder for ${rollNo}: ${err.message ?? 'API error'}`);
    } finally {
      setSendingRolls((prev) => ({ ...prev, [rollNo]: false }));
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
                  ...
                  {riskSummary.top_high_risk.map((item) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          id={`notify-btn-${item.roll_no}`}
                          onClick={() => handleNotifyStudent(item.roll_no)}
                          disabled={notifiedRolls[item.roll_no] || sendingRolls[item.roll_no]}
      ...
      {/* Toast Notification Banner — color-coded by delivery status */}
      {toastMessage && (
        <div className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-2xl shadow-2xl border flex items-center gap-2.5 text-xs animate-in slide-in-from-bottom-5 ${
          toastType === 'success'
            ? 'bg-emerald-900 text-white border-emerald-700'
            : toastType === 'simulated'
            ? 'bg-slate-900 text-white border-amber-700'
            : 'bg-red-950 text-white border-red-800'
        }`}>
          {toastType === 'success' && <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />}
          {toastType === 'simulated' && <Bell className="w-4 h-4 text-amber-400 shrink-0" />}
          {toastType === 'error' && <AlertTriangle className="w-4 h-4 text-red-400 shrink-0" />}
